...
export default function Lista() {
    ...
    const fetchVideos = async () => {
      try {
        ...
        if (!userEmail || !listaId) {
          alert('Faltan datos de usuario o lista');
          setVideos([]);
          return;
        }
        ...
        alert('No se pudieron cargar los videos de la lista');
        console.error(error);
      }
    };
    fetchVideos();
  ...
  return (
    <div style={styles.container}>
      <div style={styles.title}>Lista</div>
      <div style={styles.scrollContainer}>
        {videos.length === 0 ? (
          <div style={styles.emptyMessage}>No tienes videos todavía.</div>
        ) : (
          videos.map((item) => (
            <Video
              key={item.id}
